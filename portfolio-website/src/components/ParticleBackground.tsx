...
import tailwindConfig from '../../tailwind.config.mjs'; // Adjust path as needed
...
const ParticleBackground = () => {
  ...
  useEffect(() => {
    initParticlesEngine(async (engine: any) => { // Added type any to engine
  ...
  const particleOptions = useMemo(() => {
    ...
    const primaryColor = colors.primary?.DEFAULT || '#7F7EFF';
    const foregroundDark = colors.foreground?.dark || '#EBEBEB';
    const foregroundLight = colors.foreground?.light || '#1A1A1A';
